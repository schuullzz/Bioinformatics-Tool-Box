...
void transcription(string file)
{
	...
	for (int x = 0; x < (int)nucleoDNA.length(); x++)
	{
		if (x % 3 == 0 && x != 0)
		{
...
void translation(string file)
{
	...
	//Breaks up DNA into codons.
	for (int x = 0; x < (int)nucleoRNA.length(); x++)
	{
		if (x % 3 == 0 && x != 0)
		{
...
{
	for (int x = 0; x < (int)nucleotides.length(); x++)
	{
...
vector <int> indexStartCodon(vector<string> nucleotides)
{
	...
	for (int x = 0; x < (int)nucleotides.size(); x++)
	{
...
int findEndIndex(vector<int> stopIndex, int indexStart)
{
	...
	for (int x = 0; x < (int)stopIndex.size(); x++)
	{
...
string translation(vector<string> nucleotides, int start, int end)
{
	...
	for (int x = start * 3; x < (int)DNA.length(); x++)
	{
		switch (DNA[x])
		{
		case 'A': RNA += 'U';
